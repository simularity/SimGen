...
bt_operator(context_node_pair) --> [^].  % runs as long as a node in another context is running, then does what the foreign node does. fails if node not running.
...
% all running nodes)
% Need a node that establishes a constraint, a la a flow language.
bt_operator(expression) --> ['!'].
...
% need decoration that runs as long as the thing it calls, just for the
% events
...
Functional nodes assert a value as a probablistic function of other
 values. They are active when running. they run until terminated..

 There are boundary condition nodes that fail when outside some domain.
